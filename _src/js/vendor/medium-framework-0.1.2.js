...
framework_util = function () {
  var template, Util, _ = {}, fn = {
      ...
      get: function (o, path, defaultValue) {
        var pathtree = o.hasOwnProperty(path) ? [path] : Util.isArray(path) ? path : path.split(/[\[\].:]+/), multi, fn = function (o, path, i, pathtree) {
      ...
      isEqual: function (a, b, args) {
        if (!Util.isObject(a) && !Util.isFunction(b) && typeof a !== typeof b)
          return false;
        if (Util.isBoolean(b))
          return !!a === b;
...
framework_props = function (_) {
  var Props = {
    ...
    init: function (namespace, defaults, history) {
      ...
      var props = this.props[namespace], data = this.data[namespace], fn = {
          ...
          history: function (prop, value, isTrigger) {
            if (!history)
              return;
            Props.history.append(isTrigger ? [
              namespace,
              prop,
              value
            ] : [
    ...
    history: {
      ...
      reload: function () {
        if (!Props.history.log.pre.length)
          return;
        Props.history.reloading = true;
        console.log('History:', Props.history.log.pre.length);
        var pre = Props.history.log.pre.slice(), onIterate = function () {
            var json = pre.shift(), args = JSON.parse(json, function (key, value) {
        ...
        event: function (types, fn) {
          ...
          types.split(' ').forEach(function (typespace) {
            ...
            var type = Events.fn.type.e(typespace), namespace = Events.fn.type.ns(typespace), events = _.filter(Events.list, function (e, i) {
                ...
                if (e.$el !== this)
                  return;
                return (!type || e.type === type) && (!namespace || e.ns.slice(-namespace.length) === namespace) && (!fn || e.fn === fn);
              }.bind(this));
            for (var i in events) {
  ...
  var View = {
    ...
    $el: {
      ...
      bind: function (rebind) {
        ...
        if (rebind === true || !this.$el || !this.$el.length)
          View.$el.set(this);
